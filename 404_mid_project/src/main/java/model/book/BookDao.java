...
public class BookDao {
	...
	// 해당 숙소의 객실 목록
	/*
	 * public List<RoomDto> getRoomByStayNum(int stayNum){ List<RoomDto> list = new
	 * ArrayList<>(); Connection conn = null; PreparedStatement pstmt = null;
	 * ResultSet rs = null;
	 * 
	 * String sql = """ SELECT room_num, room_name FROM room WHERE room_stay_num = ?
	 * """; try { conn = DBConnector.getConn(); pstmt = conn.prepareStatement(sql);
	 * rs = pstmt.executeQuery(); while(rs.next()) { RoomDto dto = new RoomDto();
	 * dto.setRoomNum(rs.getInt("room_num"));
	 * dto.setRoomName(rs.getString("room_name")); list.add(dto); } } catch
	 * (Exception e) { e.printStackTrace(); } finally { DBConnector.close(rs, pstmt,
	 * conn); } return list; }
	 */
